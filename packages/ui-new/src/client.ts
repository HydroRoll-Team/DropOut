...
import type { Account, AccountSummary, ContentSearchResult, ContentVersion, DetectedLauncher, DeviceCodeResponse, FabricGameVersion, FabricLoaderEntry, FabricLoaderVersion, FileInfo, ForgeVersion, GithubRelease, ImportableInstance, InstalledFabricVersion, InstalledForgeVersion, InstalledVersion, Instance, InstanceRepairResult, JavaCatalog, JavaDownloadInfo, JavaInstallation, LauncherConfig, Message, MigrationResult, ModInfo, ModelInfo, PastebinResponse, PendingJavaDownload, Version, VersionMetadata } from "@/types"
...
export function changeInstanceDirectory(instanceId: string, newDir: string, migrateFiles: boolean): Promise<Instance> {
    return invoke<Instance>("change_instance_directory", {
        instanceId, newDir, migrateFiles
    })
}

export function checkVersionInstalled(instanceId: string, versionId: string): Promise<boolean> {
...
export function convertModLoader(instanceId: string, targetLoader: string, loaderVersion: string): Promise<string> {
    return invoke<string>("convert_mod_loader", {
        instanceId, targetLoader, loaderVersion
    })
}
...
export function deleteVersion(instanceId: string, versionId: string): Promise<void> {
...
export function downloadAdoptiumJava(majorVersion: number, imageType: string, customPath: string | null): Promise<JavaInstallation> {
    return invoke<JavaInstallation>("download_adoptium_java", {
        majorVersion, imageType, customPath
    })
}

export function downloadContentFile(instanceId: string, url: string, fileName: string, subfolder: string): Promise<string> {
    return invoke<string>("download_content_file", {
        instanceId, url, fileName, subfolder
    })
}

export function duplicateInstance(instanceId: string, newName: string): Promise<Instance> {
    return invoke<Instance>("duplicate_instance", {
        instanceId, newName
    })
}

export function exportInstance(instanceId: string, archivePath: string): Promise<string> {
    return invoke<string>("export_instance", {
        instanceId, archivePath
    })
}

export function fetchAdoptiumJava(majorVersion: number, imageType: string): Promise<JavaDownloadInfo> {
...
export function getContentVersions(projectId: string, gameVersions: string[], loaders: string[]): Promise<ContentVersion[]> {
...
export function getFabricLoadersForVersion(gameVersion: string): Promise<FabricLoaderEntry[]> {
...
export function getForgeVersionsForGame(gameVersion: string): Promise<ForgeVersion[]> {
...
export function getRecommendedJava(requiredMajorVersion: number | null): Promise<JavaInstallation | null> {
...
export function getVersionJavaVersion(instanceId: string, versionId: string): Promise<number | null> {
    return invoke<number | null>("get_version_java_version", {
        instanceId, versionId
    })
}

export function getVersionMetadata(instanceId: string, versionId: string): Promise<VersionMetadata> {
...
export function importFromLauncher(sourcePath: string, newName: string | null): Promise<Instance> {
    return invoke<Instance>("import_from_launcher", {
        sourcePath, newName
    })
}

export function importInstance(archivePath: string, newName: string | null): Promise<Instance> {
    return invoke<Instance>("import_instance", {
        archivePath, newName
    })
}

export function installFabric(instanceId: string, gameVersion: string, loaderVersion: string): Promise<InstalledFabricVersion> {
    return invoke<InstalledFabricVersion>("install_fabric", {
        instanceId, gameVersion, loaderVersion
    })
}

export function installForge(instanceId: string, gameVersion: string, forgeVersion: string): Promise<InstalledForgeVersion> {
    return invoke<InstalledForgeVersion>("install_forge", {
        instanceId, gameVersion, forgeVersion
    })
}

export function installVersion(instanceId: string, versionId: string): Promise<void> {
    return invoke<void>("install_version", {
        instanceId, versionId
    })
}

export function isFabricInstalled(instanceId: string, gameVersion: string, loaderVersion: string): Promise<boolean> {
    return invoke<boolean>("is_fabric_installed", {
        instanceId, gameVersion, loaderVersion
    })
}

export function listInstalledFabricVersions(instanceId: string): Promise<string[]> {
    return invoke<string[]>("list_installed_fabric_versions", {
        instanceId
    })
}

export function listInstalledVersions(instanceId: string): Promise<InstalledVersion[]> {
    return invoke<InstalledVersion[]>("list_installed_versions", {
        instanceId
    })
}

export function listInstanceDirectory(instanceId: string, folder: string): Promise<FileInfo[]> {
...
export function scanLauncherInstances(instancesDir: string): Promise<ImportableInstance[]> {
    return invoke<ImportableInstance[]>("scan_launcher_instances", {
        instancesDir
    })
}

export function searchContent(query: string, projectType: string, gameVersions: string[], loaders: string[], sortBy: string, offset: number, limit: number): Promise<ContentSearchResult> {
    return invoke<ContentSearchResult>("search_content", {
        query, projectType, gameVersions, loaders, sortBy, offset, limit
    })
}
...
export function startGame(instanceId: string, versionId: string): Promise<string> {
