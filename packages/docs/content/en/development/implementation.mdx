...
- **Live sample**: `MemoryState` refreshes only physical-memory data through `sysinfo`; readiness and launch independently take a fresh sample.
- **Workload target**: vanilla targets 3072 MB. Modded instances start at 3072 MB plus 32 MB per enabled archive, rounded up to 256 MB and capped at 12 GB.
- **System reserve**: the automatic heap excludes 15% of installed RAM, clamped to 1536–4096 MB. If the target does not fit, `Xmx` is rounded down in 256 MB steps; `Xms` starts at no more than 1024 MB so the JVM does not eagerly commit the full heap.
- **Precedence**: instance override → automatic calculation or global manual range. A manual range that approaches or exceeds currently available RAM is reported as constrained or critical.
- **Safety**: `get_launch_readiness` and `start_game` call the same resolver. Critical pressure blocks both direct and UI-triggered launches.
...
The home command center calls `get_launch_readiness(instance_id, version_id)` before enabling launch. The command checks version metadata/client files and calls the same Java and memory resolvers used by `start_game`. Java follows instance override → global path → preferred managed runtime → automatic detection; memory follows instance override → automatic calculation or global manual range. This keeps the displayed launch decision and real process arguments consistent.
...
| `get_memory_recommendation` | Invoke | Return live RAM, workload, recommendation, applied policy, source, and pressure for an instance |
...
The frontend uses React hooks for local component state and Zustand stores for shared application state.
...
#### Frontend View Routing (React Router)
DropOut uses React Router route modules under `packages/ui/src/pages`, with shared UI state kept in React hooks and model stores.

```typescript
// ui/src/pages/routes.ts
