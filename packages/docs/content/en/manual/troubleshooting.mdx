...
- **Ask diagnostic assistant** opens a recovery workspace with a redacted evidence preview. Logs remain detached unless you enable **Attach to next request**.
...
**1. Check the automatic memory panel:**
- Open Settings → General → JVM Optimization
- Refresh the memory sample and check the workload and pressure status
- Close memory-heavy applications if the status is constrained or critical
- Verify that the active instance is not forcing an outdated custom override

**2. Increase a manual allocation only when needed:**
- Disable Automatic memory or enable an instance override
- Raise Maximum Memory gradually in 512 MB steps
- Keep enough live headroom for the OS; the panel will warn or block unsafe values
