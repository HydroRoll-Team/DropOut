...
### Modern UI
State-driven launch command center, scalable instance library, light/dark themes, and responsive layouts at the supported desktop window sizes.

### Safe Configuration
Locally bundled Monaco JSON editor with schema diagnostics, formatting, keyboard save, and unsaved-change protection.
...
### Desktop Lifecycle
System tray controls keep recent launches, download progress, crash status, and explicit exit available while the main window is hidden.
...
DropOut is open source software licensed under the GNU Affero General Public License v3.0 or later.
