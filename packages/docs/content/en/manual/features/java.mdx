...
Automatic memory is enabled by default in **Settings → General → JVM Optimization**. Before every launch, DropOut:

1. samples currently available physical memory;
2. counts enabled `.jar`, `.zip`, and `.litemod` files in the active instance;
3. selects a workload target for vanilla, light, moderate, or heavy modded play;
4. protects 15% of installed RAM for the system, with a 1.5–4 GB reserve; and
5. lowers the heap in 256 MB steps when the full target does not safely fit.

The settings panel shows the live sample, next-launch heap, protected reserve, workload, and pressure status. The home readiness check uses the same calculation as the real launch command. Critical pressure blocks launch instead of allowing a likely system-wide out-of-memory failure.

Turn off **Automatic memory** to set a fixed global `Xms`/`Xmx` range. An instance can also enable **Custom memory for this instance**; that instance override has priority over both global modes. Clearing the override makes the instance inherit the global policy again.
