...
### 3.1 内存分配方案
- **实时采样**：`MemoryState` 通过 `sysinfo` 只刷新物理内存数据；就绪检查和实际启动都会独立获取最新采样。
- **负载目标**：原版目标为 3072 MB。模组实例从 3072 MB 起，每个已启用归档增加 32 MB，向上取整到 256 MB，最高 12 GB。
- **系统预留**：自动堆内存会排除已安装内存的 15%，并限制在 1536–4096 MB。目标无法安全装入时，`Xmx` 以 256 MB 向下调整；`Xms` 最高从 1024 MB 起步，避免 JVM 立即提交完整堆内存。
- **优先级**：实例覆盖 → 自动计算或全局手动范围。手动值接近或超过当前可用内存时会标记为受限或临界。
- **安全性**：`get_launch_readiness` 与 `start_game` 调用同一解析器，临界压力会阻止直接调用和 UI 启动。
...
首页指挥台会在启用启动操作前调用 `get_launch_readiness(instance_id, version_id)`。该命令检查版本元数据和客户端文件，并调用与 `start_game` 相同的 Java 与内存解析器。Java 使用实例覆盖 → 全局路径 → 首选托管运行时 → 自动检测；内存使用实例覆盖 → 自动计算或全局手动范围，确保页面显示的启动判断与真实进程参数一致。

| 命令 / 事件 | 类型 | 用途 |
| :--- | :--- | :--- |
| `get_launch_readiness` | Invoke | 返回文件安装状态、所需 Java 主版本和已解析的兼容运行时 |
| `get_memory_recommendation` | Invoke | 返回实例的实时内存、负载、建议值、实际策略、来源和压力状态 |
...
前端使用 React hooks 管理局部组件状态，并使用 Zustand store 管理跨视图共享状态。
...
#### 前端视图路由 (React Router)
DropOut 使用 `packages/ui/src/pages` 下的 React Router 路由模块，并通过 React hooks 和 model stores 管理共享 UI 状态。
