...
**设置 → 常规 → JVM 优化** 默认启用自动内存。每次启动前，DropOut 会：

1. 读取当前可用的物理内存；
2. 统计活动实例中已启用的 `.jar`、`.zip` 与 `.litemod` 文件；
3. 按原版、轻量、中型或大型模组实例确定目标值；
4. 为系统保护已安装内存的 15%，预留范围为 1.5–4 GB；
5. 在完整目标无法安全装入时，以 256 MB 为档位自动下调游戏堆内存。

设置面板会显示实时采样、下次启动堆内存、系统预留、实例负载和内存压力。首页就绪检查与实际启动命令使用同一套计算；当压力达到临界值时会阻止启动，避免系统进入内存耗尽状态。

关闭**自动内存**后，可以配置固定的全局 `Xms`/`Xmx`。实例也可以启用**为此实例自定义内存**，该覆盖值优先于两种全局模式；关闭实例覆盖即可重新继承全局策略。
