...
<div class="h-full flex flex-col p-6 overflow-hidden">
  <div class="flex items-center justify-between mb-6">
    <h2
      class="text-3xl font-black bg-clip-text text-transparent bg-gradient-to-r dark:from-white dark:to-white/60 from-gray-900 to-gray-600"
    >
      Settings
    </h2>
  </div>
  ...
  <div class="flex-1 overflow-y-auto pr-2 space-y-6 custom-scrollbar pb-10">
    ...
    >
      <h3
        class="text-xs font-bold uppercase tracking-widest dark:text-white/40 text-black/40 mb-6 flex items-center gap-2"
      >
        Appearance
      </h3>
      ...
      <div class="space-y-4">
        <div>
          <label
            ...
            class="block text-sm font-medium dark:text-white/70 text-black/70 mb-3"
            >Custom Background Image</label
          >

          <div id="custom-background" class="flex items-center gap-6">
            <!-- Preview -->
            <div
              class="w-40 h-24 rounded-xl overflow-hidden dark:bg-black/50 bg-gray-200 border dark:border-white/10 border-black/10 relative group shadow-lg"
            >
              ...
              {:else}
                <div
                  class="w-full h-full bg-gradient-to-br from-emerald-900 via-zinc-900 to-indigo-950 opacity-100"
                ></div>
                <div
                  class="absolute inset-0 flex items-center justify-center text-[10px] text-white/50 bg-black/20"
                >
                  Default Gradient
                </div>
              {/if}
          ...
          <p class="text-xs dark:text-white/30 text-black/40 mt-3">
            Select an image from your computer to replace the default gradient
            background. Supported formats: PNG, JPG, WEBP, GIF.
          </p>
        </div>
        ...
        <div class="pt-4 border-t dark:border-white/5 border-black/5 space-y-4">
          <div class="flex items-center justify-between">
            <div>
              <h4
                ...
                id="visual-effects-label"
              >
                Visual Effects
              </h4>
              <p class="text-xs dark:text-white/40 text-black/50 mt-1">
            ...
            <button
              ...
              onclick={() => {
                settingsState.settings.enable_visual_effects =
                  !settingsState.settings.enable_visual_effects;
                settingsState.saveSettings();
              }}
              class="w-11 h-6 rounded-full transition-colors duration-200 ease-in-out relative focus:outline-none {settingsState
                .settings.enable_visual_effects
                ? 'bg-indigo-500'
                : 'dark:bg-white/10 bg-black/10'}"
            >
              <div
                class="absolute top-1 left-1 bg-white w-4 h-4 rounded-full shadow-sm transition-transform duration-200 ease-in-out {settingsState
                  .settings.enable_visual_effects
                  ? 'translate-x-5'
                  : 'translate-x-0'}"
              ></div>
          ...
          <div class="flex items-center justify-between">
            <div>
              <h4
                class="text-sm font-medium dark:text-white/90 text-black/80"
                id="gpu-acceleration-label"
              >
                GPU Acceleration
              </h4>
              <p class="text-xs dark:text-white/40 text-black/50 mt-1">
                Enable GPU acceleration for the interface. (Default: Off,
                Requires Restart)
              </p>
            </div>
            <button
              aria-labelledby="gpu-acceleration-label"
              onclick={() => {
                settingsState.settings.enable_gpu_acceleration =
                  !settingsState.settings.enable_gpu_acceleration;
                settingsState.saveSettings();
              }}
              class="w-11 h-6 rounded-full transition-colors duration-200 ease-in-out relative focus:outline-none {settingsState
                .settings.enable_gpu_acceleration
                ? 'bg-indigo-500'
                : 'dark:bg-white/10 bg-black/10'}"
            >
              <div
          ...
          <div
            class="flex items-center justify-between pt-4 border-t dark:border-white/5 border-black/5 opacity-50 cursor-not-allowed"
          >
            <div>
              <h4
                class="text-sm font-medium dark:text-white/90 text-black/80"
            ...
            <div
              class="flex items-center bg-black/5 dark:bg-white/5 rounded-lg p-1 pointer-events-none"
            >
              <button
                disabled
      ...
      <div class="space-y-4">
        ...
        {#if settingsState.javaInstallations.length > 0}
          <div class="mt-4 space-y-2">
            <p
              class="text-[10px] text-white/30 uppercase font-bold tracking-wider"
            >
                ...
                <div class="flex justify-between items-center">
                  <div>
                    <span class="text-white font-mono text-xs font-bold"
                      >{java.version}</span
                    >
                    <span class="text-white/40 text-[10px] ml-2"
                      >{java.is_64bit ? "64-bit" : "32-bit"}</span
                    >
                  </div>
                  {#if settingsState.settings.java_path === java.path}
                    <span
                      class="text-indigo-300 text-[10px] font-bold uppercase tracking-wider"
                      >Selected</span
                    >
                  {/if}
                </div>
                <div
                  class="text-white/30 text-[10px] font-mono truncate mt-1 group-hover:text-white/50"
                >
                  {java.path}
                </div>
              </button>
    ...
    >
      ...
      <div class="grid grid-cols-2 gap-6">
        <div>
          <label
            for="window-width"
            class="block text-sm font-medium text-white/70 mb-2">Width</label
          >
          <input
            id="window-width"
        ...
        <div>
          <label
            for="window-height"
            class="block text-sm font-medium text-white/70 mb-2">Height</label
          >
          <input
            id="window-height"
...
{#if settingsState.showJavaDownloadModal}
  <div
    class="fixed inset-0 z-[100] flex items-center justify-center backdrop-blur-sm bg-black/70"
  >
    <div
      class="bg-zinc-900 rounded-2xl border border-white/10 shadow-2xl w-[900px] max-w-[95vw] h-[600px] max-h-[90vh] flex flex-col overflow-hidden"
    >
      <!-- Header -->
      <div
      ...
      <div class="flex flex-1 overflow-hidden">
        ...
        <div class="w-40 border-r border-white/10 p-3 flex flex-col gap-1">
          <span
            class="text-[10px] font-bold uppercase tracking-widest text-white/30 px-2 mb-2"
            >Sources</span
          >
          ...
          >
            <div
              class="w-5 h-5 rounded bg-white/10 flex items-center justify-center text-[10px]"
            >
              M
            </div>
            Mojang
          ...
          >
            <div
              class="w-5 h-5 rounded bg-indigo-500 flex items-center justify-center text-[10px] font-bold"
            >
              A
            </div>
            Adoptium
        ...
        <div class="flex-1 flex flex-col overflow-hidden">
          ...
          <div class="flex items-center gap-3 p-4 border-b border-white/5">
            ...
            <!-- Recommended Filter -->
            <label
              class="flex items-center gap-2 text-sm text-white/60 cursor-pointer select-none"
            >
              <input
                type="checkbox"
            ...
            <!-- Image Type Toggle -->
            <div
              class="flex items-center bg-black/30 rounded-lg p-0.5 border border-white/10"
            >
              <button
                onclick={() => (settingsState.selectedImageType = "jre")}
                class="px-3 py-1.5 rounded-md text-xs font-medium transition-all {settingsState.selectedImageType ===
                'jre'
            ...
            <div class="flex-1 overflow-auto p-4">
              <div class="space-y-2">
                {#each settingsState.availableMajorVersions as version}
                  {@const isLts =
                    settingsState.javaCatalog?.lts_versions.includes(version)}
                  {@const isSelected =
                    settingsState.selectedMajorVersion === version}
                  {@const releaseInfo =
                    settingsState.javaCatalog?.releases.find(
                      (r) =>
                        r.major_version === version &&
                        r.image_type === settingsState.selectedImageType,
                    )}
                  {@const isAvailable = releaseInfo?.is_available ?? false}
                  {@const installStatus = releaseInfo
                    ? settingsState.getInstallStatus(releaseInfo)
                    : "download"}
                  ...
                  >
                    ...
                    <div class="w-14 text-center">
                      <span
                        class="text-xl font-bold {isSelected
                          ? 'text-white'
                          : 'text-white/80'}">{version}</span
                      >
                    </div>
                    ...
                    <div class="flex-1 min-w-0">
                      <div class="flex items-center gap-2">
                        <span class="text-sm text-white/70 font-mono truncate"
                          >{releaseInfo?.version ?? "--"}</span
                        >
                        {#if isLts}
                          <span
                            class="px-1.5 py-0.5 bg-emerald-500/20 text-emerald-400 text-[9px] font-bold rounded uppercase shrink-0"
                            >LTS</span
                          >
                        {/if}
                      </div>
                      {#if releaseInfo}
                        <div class="text-[10px] text-white/40 truncate mt-0.5">
                          {releaseInfo.release_name} • {settingsState.formatBytes(
                            releaseInfo.file_size,
                          )}
                        </div>
                      {/if}
                    </div>
                    ...
                    <div class="shrink-0">
                      {#if installStatus === "installed"}
                        <span
                          class="px-2 py-1 bg-emerald-500/20 text-emerald-400 text-[10px] font-bold rounded uppercase"
                          >Installed</span
                        >
                      {:else if isAvailable}
                        <span
                          class="px-2 py-1 bg-white/10 text-white/50 text-[10px] font-medium rounded"
                          >Download</span
                        >
                      {:else}
                        <span
                          class="px-2 py-1 bg-red-500/10 text-red-400/60 text-[10px] font-medium rounded"
                          >N/A</span
                        >
                      {/if}
                    </div>
        ...
        <div class="w-64 border-l border-white/10 flex flex-col">
          ...
          {#if settingsState.selectedRelease}
            <div class="flex-1 p-4 space-y-4 overflow-auto">
              <div>
                <div
                  class="text-[10px] text-white/40 uppercase tracking-wider mb-1"
                >
                  Version
                </div>
              ...
              <div>
                ...
                <div class="text-sm text-white">
                  {settingsState.formatDate(
                    settingsState.selectedRelease.release_date,
                  )}
                </div>
              </div>
              ...
              <div>
                ...
                <div class="text-sm text-white">
                  {settingsState.formatBytes(
                    settingsState.selectedRelease.file_size,
                  )}
                </div>
              </div>
              ...
              <div>
                ...
                <div class="flex items-center gap-2">
                  <span class="text-sm text-white uppercase"
                    >{settingsState.selectedRelease.image_type}</span
                  >
                  {#if settingsState.selectedRelease.is_lts}
                    <span
                      class="px-1.5 py-0.5 bg-emerald-500/20 text-emerald-400 text-[9px] font-bold rounded"
                      >LTS</span
                    >
                  {/if}
                </div>
          ...
          {:else}
            <div
              class="flex-1 flex items-center justify-center text-white/30 text-sm p-4 text-center"
            >
        ...
        <div class="border-t border-white/10 p-4 bg-zinc-900/90">
          <div class="flex items-center justify-between mb-2">
            ...
            <span class="text-xs text-zinc-400"
              >{settingsState.downloadProgress.status}</span
            >
          </div>
          ...
          <div class="mb-2">
            <div class="flex justify-between text-[10px] text-zinc-400 mb-1">
              ...
              <span
                >{Math.round(settingsState.downloadProgress.percentage)}%</span
              >
            </div>
          ...
          <div class="flex justify-between text-[10px] text-zinc-500 font-mono">
            <span>
              {settingsState.formatBytes(
                settingsState.downloadProgress.speed_bytes_per_sec,
              )}/s · ETA: {settingsState.formatTime(
                settingsState.downloadProgress.eta_seconds,
              )}
            </span>
            <span>
              {settingsState.formatBytes(
                settingsState.downloadProgress.downloaded_bytes,
              )} /
              {settingsState.formatBytes(
                settingsState.downloadProgress.total_bytes,
              )}
            </span>
          </div>
        </div>
          ...
          <div class="flex items-center justify-between">
            <div class="flex items-center gap-3">
              ...
              <span class="text-sm text-amber-200">
                {settingsState.pendingDownloads.length} pending download(s) can be
                resumed
              </span>
            </div>
            <button
      ...
      <!-- Footer Actions -->
      <div
        class="flex items-center justify-between p-4 border-t border-white/10 bg-black/20"
      >
        <button
          onclick={() => settingsState.refreshCatalog()}
          disabled={settingsState.isLoadingCatalog ||
            settingsState.isDownloadingJava}
        ...
        >
          <svg
            class="w-4 h-4 {settingsState.isLoadingCatalog
              ? 'animate-spin'
              : ''}"
            fill="none"
            stroke="currentColor"
        ...
        <div class="flex gap-3">
          ...
          {:else}
            {@const isInstalled = settingsState.selectedRelease
              ? settingsState.getInstallStatus(
                  settingsState.selectedRelease,
                ) === "installed"
              : false}
            <button
              onclick={() => settingsState.closeJavaDownloadModal()}
            ...
            <button
              ...
              disabled={!settingsState.selectedRelease?.is_available ||
                settingsState.isLoadingCatalog ||
                isInstalled}
