...
>
  ...
  <nav class="flex-1 w-full flex flex-col gap-1 px-3">
    <!-- Nav Item Helper -->
    {#snippet navItem(view: ViewType, Icon: any, label: string)}
      <button
      ...
      >
        ...
        {#if uiState.currentView === view}
          <div
            class="absolute left-0 top-1/2 -translate-y-1/2 w-0.5 h-4 bg-black dark:bg-white rounded-r-full hidden lg:block"
          ></div>
        {/if}
      </button>
    {/snippet}
