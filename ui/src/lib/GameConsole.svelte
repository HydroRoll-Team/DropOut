<script lang="ts">
  ...
  let filteredLogs = $derived(
    logsState.logs.filter((log) => {
      ...
      if (!showError && (log.level === "error" || log.level === "fatal"))
        return false;
    ...
    try {
      ...
      if (path) {
        ...
        logsState.addLog(
          "info",
          "Console",
          `Exported ${filteredLogs.length} logs to ${path}`,
        );
      }
    } catch (e) {
  ...
  async function uploadLogs() {
    try {
      ...
      logsState.addLog(
        "info",
        "Console",
        `Uploading ${filteredLogs.length} logs...`,
      );

      const response = await invoke<{ url: string }>("upload_to_pastebin", {
        content,
      });
      ...
      logsState.addLog(
        "info",
        "Console",
        `Logs uploaded successfully: ${response.url}`,
      );
...
<div
  class="absolute inset-0 flex flex-col bg-[#1e1e1e] text-zinc-300 font-mono text-xs overflow-hidden"
>
  <!-- Toolbar -->
  <div
    class="flex flex-wrap items-center justify-between p-2 bg-[#252526] border-b border-[#3e3e42] gap-2"
  >
    <div class="flex items-center gap-3">
      <h3 class="font-bold text-zinc-100 uppercase tracking-wider px-2">
        Console
      </h3>
      ...
      <!-- Level Filters -->
      <div
        class="flex items-center bg-[#1e1e1e] rounded border border-[#3e3e42] overflow-hidden"
      >
        <button
          class="px-2 py-1 hover:bg-[#3e3e42] transition-colors {showInfo
            ? 'text-blue-400'
            : 'text-zinc-600'}"
          onclick={() => (showInfo = !showInfo)}
          title="Toggle Info">Info</button
        >
        <div class="w-px h-3 bg-[#3e3e42]"></div>
        <button
          class="px-2 py-1 hover:bg-[#3e3e42] transition-colors {showWarn
            ? 'text-yellow-400'
            : 'text-zinc-600'}"
          onclick={() => (showWarn = !showWarn)}
          title="Toggle Warnings">Warn</button
        >
        <div class="w-px h-3 bg-[#3e3e42]"></div>
        <button
          class="px-2 py-1 hover:bg-[#3e3e42] transition-colors {showError
            ? 'text-red-400'
            : 'text-zinc-600'}"
          onclick={() => (showError = !showError)}
          title="Toggle Errors">Error</button
        >
        <div class="w-px h-3 bg-[#3e3e42]"></div>
        <button
          class="px-2 py-1 hover:bg-[#3e3e42] transition-colors {showDebug
            ? 'text-purple-400'
            : 'text-zinc-600'}"
          onclick={() => (showDebug = !showDebug)}
          title="Toggle Debug">Debug</button
        >
      </div>
      ...
      <div class="relative group">
        ...
        />
        <svg
          class="w-3.5 h-3.5 text-zinc-500 absolute left-2.5 top-1.5"
          fill="none"
          stroke="currentColor"
          viewBox="0 0 24 24"
          ><path
    ...
    <div class="flex items-center gap-2">
      <!-- Log count indicator -->
      <span class="text-zinc-500 text-[10px] px-2"
        >{filteredLogs.length} / {logsState.logs.length}</span
      >
  ...
  >
    {#each filteredLogs as log (log.id)}
      <div
        class="flex gap-2 leading-relaxed hover:bg-[#2a2d2e] px-1 rounded-sm group"
      >
        <!-- Timestamp -->
        <span
          class="text-zinc-500 shrink-0 select-none w-20 text-right opacity-50 group-hover:opacity-100"
          >{log.timestamp.split(".")[0]}</span
        >

        <!-- Source & Level -->
        <div
          class="flex shrink-0 min-w-[140px] gap-1 justify-end font-bold select-none truncate"
        >
          <span class="text-zinc-500 truncate max-w-[90px]" title={log.source}
            >{log.source}</span
          >
        ...
        <!-- Message -->
        <div
          class="flex-1 break-all whitespace-pre-wrap text-zinc-300 min-w-0 {getMessageColor(
            log,
          )}"
        >
